...
const addNeuralEventHandlers = () => {
  ...
  document.addEventListener('file-opened', (e) => {
    if (isActive && neuralOverlay) {
      const filePath = e.detail?.filePath || 'unknown';
      const fileName = filePath.split('/').pop() || filePath;
      addTerminalLine(`> FILE LOADED: ${fileName}`);
  ...
  document.addEventListener('editor-content-changed', (e) => {
    if (isActive && neuralOverlay && Math.random() < 0.1) {
      addTerminalLine('> BUFFER STATE CHANGED');
    }
  });
  ...
  window.addEventListener('error', () => {
    if (isActive && neuralOverlay) {
      addTerminalLine('> ERROR DETECTED: RUNTIME EXCEPTION');
      addTerminalLine('> STACK TRACE LOGGED');
      
      // Trigger intense glitch
      if (window.chimeraGlitch) {
        window.chimeraGlitch.triggerIntenseGlitch(1000);
      }
    }
  });
  ...
  document.addEventListener('whisper-message', (e) => {
    if (isActive && neuralOverlay) {
      if (e.detail && e.detail.from === 'ai') {
        addTerminalLine('> NEURAL RESPONSE GENERATED');
      } else {
        addTerminalLine('> USER QUERY DETECTED');
      }
    }
  });
};
