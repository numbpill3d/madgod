...
// Update task runner UI
const updateTaskRunnerUI = () => {
  // Update run/stop buttons
  const runBtn = document.getElementById('run-task-btn');
  const stopBtn = document.getElementById('stop-task-btn');

  if (runBtn) {
    runBtn.disabled = taskRunning;
  }

  if (stopBtn) {
    stopBtn.disabled = !taskRunning;
  }

  // Update task select
  const taskSelect = document.getElementById('task-select');
  if (taskSelect) {
    taskSelect.disabled = taskRunning;
  }
...
const simulateTaskExecution = (task) => {
  // Generate appropriate output based on task and theme
  let outputs = [];

  if (task.name === 'start') {
    outputs = currentTheme === 'lain'
      ? [
          'Initializing electron process...',
          'Loading main window...',
          'Establishing IPC channels...',
          'Neural interface activated.',
          'CHIMERA Code Editor is now running.'
        ]
      : [
          'The ancient scrolls begin to unfurl...',
          'The window to the realm materializes...',
          'The whispers between realms grow stronger...',
          'The arcane energies flow through the conduits.',
          'CHIMERA Code Editor has awakened.'
        ];
    ...
    outputs = currentTheme === 'lain'
      ...
      : [
          'The forge of creation ignites...',
          'The elements of the realm coalesce...',
          'The binding spells are woven...',
          'The seal of authenticity is applied...',
          'The artifact is complete. Seek it in the ./dist chamber.'
        ];
    ...
    outputs = currentTheme === 'lain'
      ...
      : [
          'The trials of truth begin...',
          'Testing the neural pathways of the mind...',
          'Examining the scrolls of knowledge...',
          'Inspecting the visible manifestations...',
          'All trials completed successfully. The code is 87% proven.'
        ];
  } else {
    // Generic outputs for other tasks
    outputs = currentTheme === 'lain'
      ? [
          'Executing task...',
          'Processing...',
      ...
      : [
          ...
          'The arcane energies flow...',
          'The patterns reveal themselves...',
          'The final runes are inscribed...',
  ...
  outputs.forEach((output, index) => {
    setTimeout(() => {
      // Only add output if task is still running
      if (taskRunning && currentTask === task.name) {
        addTaskOutput(`${output}\n`);

        // If this is the last output, complete the task
...
const completeTask = () => {
  ...
  addTaskOutput(currentTheme === 'lain'
    ? `\n> Task "${currentTask}" completed successfully.\n`
    : `\n> The ${currentTask} constellation has fulfilled its purpose.\n`
  );
  ...
  if (window.chimeraWhisper) {
    const message = currentTheme === 'lain'
      ? `Task "${currentTask}" completed successfully.`
      : `The ${currentTask} constellation has completed its celestial journey.`;
