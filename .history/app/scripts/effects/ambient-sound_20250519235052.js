...
const playSound = (soundName) => {
  ...
  // Get the appropriate sound based on theme
  const effectUrl = sounds[currentTheme].effects[soundName];
  ...
  // For demonstration, log that we would play the sound
  // In a real app with assets available, we would use Howler.js
  console.log(`Playing sound effect: ${effectUrl}`);

  /*
  In a real implementation with actual sound files:

  // Create the sound instance if it doesn't exist
  if (!effectInstances[effectUrl]) {
    effectInstances[effectUrl] = new Howl({
      src: [effectUrl],
      volume: volume
    });
  }
  ...
  // Play the sound
  effectInstances[effectUrl].play();
  */
};

// Set up event listeners for application interactions
const setupSoundEvents = () => {
  // Play keypress sound on monaco editor keypress
  // This would require integration with the editor's key events

  // Play save sound when a file is saved
  ...
  // Play whisper sound when a message is received
  document.addEventListener('whisper-message', (e) => {
    if (e.detail && e.detail.from === 'ai') {
      playSound('whisper');
    }
  });

  // Play open/close sounds for files
  ...
  // Play theme-specific effects
  document.addEventListener('glitch-effect', () => {
    if (currentTheme === 'lain') {
      playSound('glitch');
    }
  });

  document.addEventListener('magic-effect', () => {
  ...
  // Simulate keypresses for keypress sounds
  document.addEventListener('keydown', (e) => {
    // Don't play for modifier keys or function keys
    if (e.key.length === 1 || e.key === 'Enter' || e.key === 'Backspace' || e.key === 'Space') {
      if (currentTheme === 'lain') {
        playSound('keypress');
      } else {
        // Less frequent scroll sounds for Morrowind theme (20% chance)
        if (Math.random() < 0.2) {
          playSound('scroll');
        }
      }
